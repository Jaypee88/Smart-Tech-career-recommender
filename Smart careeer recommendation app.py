...
def load_data(path):
    ...
    expected = ['Skills', 'Interests', 'Education', 'Recommended_Career']
    for c in expected:
...
recommended_df = pd.DataFrame(columns=['Career', 'Skills Match %', 'Interests Match %',
                            'Education Match %', 'Final Score %'])


#Defined a function that will  do the recommendation
def recommend(df, tfidf_skills, tfidf_interests, tfidf_edu,
                  skills_matrix, interests_matrix, edu_matrix,
                  user_skills, user_interests, user_edu,
    ...
    final_score = (weights[0] * skills_sim) + (weights[1] * interests_sim) + (weights[2] * edu_sim)
...
st.markdown("Use the sidebar to navigate. This app recommends tech careers based on skills, interests and education.")
...
with st.sidebar.expander('Dataset'):
        ...
        if Dataset:
            df = load_data("AI-based Career Recommendation System.csv")
        else:
...
weights_skill = st.sidebar.slider('Skills weight', 0.0, 1.0, 0.5)
weights_interest = st.sidebar.slider('Interests weight', 0.0, 1.0, 0.2)
weights_edu = st.sidebar.slider('Education weight', 0.0, 1.0, 0.3)
w_sum = weights_skill + weights_interest + weights_edu
if w_sum == 0:
    weights = (0.5, 0.2, 0.3) #A constant value  incase  user  decides not to choose he weight

else:
    #This is to normalise the weight so that no matter the user input they all should all sum  to 1
    weights = (weights_skill/w_sum, weights_interest/w_sum, weights_edu/w_sum)
    st.write(f"Normalized Weights: Skills={weights[0]:.2f}, Interests={weights[1]:.2f}, Education={weights[2]:.2f}")


if  page == 'Home':
    st.header("Homepage")
    st.write('Welcome this app recommends a career path to users based on their interests, skills and education')
...
#Added a page that allows  a user to know about the dataset and explore for some of its features
    ...
    st.write(f'COLUMNS: {len(df.columns)}') #display the columnd
...
#Defined the  page for recommendation
    ...
    if st.button('Recommend Career'):
        if not user_skills or not user_interests or not user_education:
            st.error('Please fill out the required fields')

        else:
            recs = recommend(df, tfidf_skills, tfidf_interests, tfidf_edu,
                             skills_matrix, interests_matrix, edu_matrix,
                             user_skills, user_interests, user_education,
